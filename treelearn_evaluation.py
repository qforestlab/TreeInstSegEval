...
class TreeLearnEvaluation(BaseEvaluation):
    ...
    def __init__(self, data_base_dir= "/Stor1/wout/BenchmarkPaper/data/", dataset="WYTHAM", use_cached_calculations=True, cache_calculations=True, debug=False, new_version=True):
        ...
        if self.new_version:
            self.method = "treelearn"
        else:
            self.method = "treelearn_old"
        super(TreeLearnEvaluation, self).__init__(data_base_dir, dataset, use_cached_calculations, cache_calculations, debug)
        return
